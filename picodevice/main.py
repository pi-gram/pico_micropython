...
display = PicoGraphics(display=DISPLAY_PICO_DISPLAY, pen_type=PEN_P8, rotate=180) #ah! the 180 moved the x & y all sideways!
...
def draw_barrier(barrier_type, pen, x, y):
    display.set_pen(pen)
...
#            print("shoty:"+str(shoty)+" ufoy:"+str(ufoy))
            if shoty >= ufoy:
                if shoty < ufoy + 20: #still no 100% happy with this, but it'll do for now
    ...
    #these will remain solid for now
    #we should have collision detection to make sure we cannot shoot through them!
    draw_barrier(barrier, CYAN, 55, 10)
    draw_barrier(barrier, CYAN, 55, 55)
    draw_barrier(barrier, CYAN, 55, 100)
